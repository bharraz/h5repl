...
- Virtual Datasets and Groups. When a file is opened (through `h5open`), an extension of `h5py.File` found in goldh5file.py is instead used. 

  - One key Virtual dataset is "params" where all the items from the experiment params (usually under 'expid') is added as groups/datasets to the opened file
  - Another is the population datasets. These ..
