...
def _first_peak_time(x, y, inversed=False):
    """
    Estimate pi-time (x at first peak, or trough if inversed) by scanning
    x-sorted data for two consecutive descents (or ascents).
    Falls back to x at argmax / argmin if no clear turning point is found.
    """
    sorted_xy = sorted(zip(x, y))       # sorted by x
    ys = [v for _, v in sorted_xy]
    fallback_idx = int(np.argmin(ys) if inversed else np.argmax(ys))
    pi_time = sorted_xy[fallback_idx][0]

    if len(sorted_xy) > 3:
        for i in range(2, len(sorted_xy)):
            y0, y1, y2 = sorted_xy[i-2][1], sorted_xy[i-1][1], sorted_xy[i][1]
            descent = (y0 > y1) and (y1 > y2)
            ascent  = (y0 < y1) and (y1 < y2)
            if (descent and not inversed) or (ascent and inversed):
                pi_time = sorted_xy[i-2][0]   # x at the turning point
                break
    return pi_time
...
def fit_rabi(series, *, amp=None, omega=None, offset=None):
...
def fit_decaying_cosine(series, *, amp=None, omega=None, phi=None, tau=None, offset=None):
    ...
    _tau = float(pi_time * 50) if tau is None else float(tau)
...
def fit_lorentzian(series, *, center=None, floor=None, amp=None, fwhm=None):
...
def fit_gaussian(series, *, center=None, floor=None, amp=None, fwhm=None):
...
def fit_exp_decay(series, *, floor=None, amp=None, tau=None):
    ...
    _tau   = float(np.max(x) / 2)         if tau   is None else float(tau)
...
def fit_ramsey_phase(series, *, amp=None, offset=None, delay=None):
...
def fit_ramsey_time(series, *, amp=None, omega=None, offset=None, delay=None, tau=None):
...
def fit_linear(series, *, slope=None, intercept=None):
...
def fit_quadratic(series, *, scale=None, center=None, offset=None):
