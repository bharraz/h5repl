"""
Fit shorthand functions for common curve shapes.

Each function:
  - accepts a Series (build one first: Series(x, y, yerr=...))
  - reads series.x / series.y (and .yerr if present)
  - auto-guesses p0 from the data
...
Usage:
    result = fit_rabi(pm1.mydata)
    result = fit_rabi(pm1.mydata, omega=np.pi * 2e4)  # override one guess
    result = fit_rabi(pm1.mydata, fix={'offset': 0.0}) # hold offset fixed at 0
...
def _coerce_series(arg):
    """Require a Series (build one first: Series(x, y, yerr=...))."""
    from .series import Series
    if isinstance(arg, Series):
        return arg
    raise TypeError(
        f"Expected a Series, got {type(arg).__name__}. "
        "Build one first: Series(x, y, yerr=...)"
    )
...
def fit_decaying_cosine(series, *, amp=None, omega=None, phi=None, tau=None,
    ...
    Override any guess:
      result = fit_decaying_cosine(pm1.s1, tau=50e-6)
      result = fit_decaying_cosine(pm1.s1, fix={'phi': 0.0})
...
def fit_lorentzian(series, *, center=None, floor=None, amp=None, fwhm=None, fix=None):
    ...
    Override any guess:
      result = fit_lorentzian(pm1.s1, center=6.3e9, fwhm=1e6)
...
def fit_gaussian(series, *, center=None, floor=None, amp=None, fwhm=None, fix=None):
    ...
    Override any guess:
      result = fit_gaussian(pm1.s1, center=6.3e9)
...
def fit_exp_decay(series, *, floor=None, amp=None, tau=None, fix=None):
    ...
    Override any guess:
      result = fit_exp_decay(pm1.s1, floor=0.05)
...
def fit_ramsey_phase(series, *, amp=None, offset=None, delay=None, fix=None):
    ...
    Override any guess:
      result = fit_ramsey_phase(pm1.s1, delay=0.25)
...
def fit_spectroscopy(series, pulse_duration=None, *, scaling=None, floor=None,
    ...
    Examples:
      result = fit_spectroscopy(pm1.s1)                         # reads t from file
      result = fit_spectroscopy(pm1.s1, 50e-6)                  # explicit
      result = fit_spectroscopy(pm1.s1, center_freq=6.834e9)    # file + override
...
def fit_linear(series, *, slope=None, intercept=None, fix=None):
    ...
    Override any guess:
      result = fit_linear(pm1.s1, slope=0.0)
...
def fit_quadratic(series, *, scale=None, center=None, offset=None, fix=None):
    ...
    Override any guess:
      result = fit_quadratic(pm1.s1, center=0.5)
